...
func TestReElection2A(t *testing.T) {
	...
	cfg.checkOneLeader()
	fmt.Printf("================\nTest (2A): leader1 disconnect, reElection success.\n=================\n")

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	fmt.Printf("================\nTest (2A): leader1 reconnect, check one leader success.\n=================\n")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	fmt.Printf("================\nTest (2A): check no leader success.\n=================\n")

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	fmt.Printf("================\nTest (2A): after check no leader success.\n=================\n")
